...
********************************************************************************/
	...
	****************************************************************************/
	...
	$agspay->SetValue("AgsPayHome","C:/Rosemary/trunk/src/rosemary/agspay");			//올더게이트 결제설치 디렉토리 (상점에 맞게 수정)
	...
	****************************************************************************/
	...
	if (!$OrdNo) alertGo("주문번호가 없습니다. ",$MY_URL); 
	...
	{ 
		if($agspay->GetResult("AuthTy") == "virtual"){
			...
			$pay_type = "C";	//카드결제
			$pay_state = "B";	//결제상태 - 카드는 결제완료 나머지는 입금대기중으로 처리


			//결제된 상품과 선택된 상품이 동일한 상품인지 제대로 결제가 됐는지 체크 한다. 
			$set_pack_num = 	$_POST["set_pack_num"];				//구매한 패키지 - 해당 변수에 값이 있으면 밑에 선택한 단과와 교재가 패키지 상품으로 분류된다.
			$set_goods_lt_num = 	$_POST["set_goods_lt_num"];			//구매한 단과
			$set_book_bo_num = 	$_POST["set_book_bo_num"];				//구매한 교재
			$g_num = 	$_POST["g_num"];				//구매한 교재

			if (!empty($set_goods_lt_num) || !empty($set_pack_num)) {
				$chk_goods = true;										// 일반 상품이든 패키지든 상품을 구매하는 경우.
			} else {
				$chk_goods = false;
			}
					...
					if (!$g_num) {
						...
						if (!$goods_nums) {
							...
							while($goods_rs = mysqli_fetch_array($goods_qry)){
								...
								$lt_in_qu[] = "('$OrdNo','$g_num','$lt_num','$price','$lt_term')";		//주문한 강좌의 선택한 단과 주문테이블에 넣을 쿼리문
								$member_lt_in_qu[] = "('$lt_term','A','$lt_num','$OrdNo','$g_num')";
							...
							$g_in_qu[] =  "('$OrdNo','$g_num','$g_discount_rate')";					// 주문한 강좌 주문테이블에 넣을 쿼리문
							$member_g_in_qu[] =  "('$OrdNo','$g_num','$mb_num',unix_timestamp(),'A')";
						}
					}
				} else {
			...
			// 디비의 가격과 실제 결제한 가격이 같은지 체크한다.
			if ($tot_price != $agspay->GetResult("Amt")) {
				alertGo("결제 가격과 실제 가격이 불일치 합니다. ",$MY_URL);
			} else {

//이상이 없다면 결제 로그를 남긴다. - ordersheet
				$Amt = $agspay->GetResult("Amt");
				if ($pay_type == "C") {	// 카드 결제일 경우 바로 입금완료이기 때문에 결제 완료일도 지금 날짜로 넣는다.
					$pay_date = mktime();
				} else {
					$pay_date = "";
				}
				...
				if (!$os_qry) {
					$t_chk = false;
					$err_msg = "결제시 오류 발생 관리자에게 문의 하세요.";
				} else {
					$zip_code = $_POST['ms_zip1']."-".$_POST['ms_zip2'];					// 우편번호
					$addr = $_POST['ms_address'];											// 받는 주소
					$addr_detail = $_POST['addr_2'];										// 받는 나머지 주소
					$osdi_name = $_POST['RcpNm'];											// 받는 사람
					$osdi_message = $_POST['Remark'];										// 배송 요청사항	
					$osdi_tel = $_POST['tel_1']."-".$_POST['tel_2']."-".$_POST['tel_3'];	// 받는 전화번호
					$osdi_hp = $_POST['hp_1']."-".$_POST['hp_2']."-".$_POST['hp_3'];		// 받는 핸드폰 번호	
					$osdi_email = $_POST['email_1']."@".$_POST['email_2'];					// 받는 이메일 
					...
					if (!$os_de_info_qry) {
						$t_chk = false;
						$err_msg = "배송정보 저장 오류 관리자에게 문의 하세요.";
					} else {
// 교재 주문서 먼저 등록
						if (!empty($set_book_bo_num)) {
							...
							if (!$ord_book_qry) {
								$t_chk = false;
								$err_msg = "교재로그 등록실패. 관리자에게 문의 하세요.";
							} 
						}
	...
	// 강좌 등록
						if ($chk_goods == true) {
							...
							if (!empty($set_pack_num)) {
								...
								if (!$ord_pack_query) {
									$t_chk = false;
									$err_msg = "패키지 로그 등록실패. 관리자에게 문의 하세요.";
								}
							}
							...
							if (!$ord_goods_qry) {
								$t_chk = false;
								$err_msg = "강좌로그 등록실패. 관리자에게 문의 하세요.";
							} else {
								// 단과 등록
								if (!empty($set_pack_num)) {
								...
								if (!$ord_lt_qry) {
									$t_chk = false;
									$err_msg = "단과로그 등록실패. 관리자에게 문의 하세요.";
								}							
							}
						}
					}
