...
if (!$write_mode) {
	...
	$privacy_in_query = mysqli_query($CONN['rosemary'],"insert into site_clause(sc_content,sc_sdate,sc_regdate) values('$contents','$start_time',unix_timestamp())");
	if (!$privacy_in_query) {
		alertback("디비 업데이트 실패.");
	}else{
		alertGo("","../index.php?mode=clause");
	}
	...
	$privacy_up_query = mysqli_query($CONN['rosemary'],"update site_clause set sc_content = '$contents',sc_sdate = '$start_time'  where sc_num = '$sc_num'");
	if (!$privacy_up_query) {
		alertback("디비 업데이트 실패.");
	} else {
		alertGo("","../index.php?mode=clause&page=$page&key=$key&searchword=$searchword");
	}
