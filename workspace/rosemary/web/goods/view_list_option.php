...
$ca_num		= $_POST['ca_num'];				// 카테고리 번호
$g_num		= $_POST['g_num'];				// 상품 번호
...
$tpl->define('content', "skin/ls/$category_skin[$ca_num]/view_list_option.html");
...
$subject_qry = "select
						...
						(select count(*) from goods_lecture_subjects C, goods_lecture_subjects_period E where C.lt_num = A.lt_num and C.lts_num = E.lts_num and E.ltsp_sample_yn = 'Y') as sample_cnt
				from 
						goods_lecture A
				where
						A.g_num = '$g_num' 
...
// 해당 강좌에 등록된 교재 정보를 가져온다.

$book_qry = "select C.* from goods_lecture_book A, goods_lecture B, book C  where B.g_num = '$g_num' and A.lt_num = B.lt_num and A.bo_num = C.bo_num group by C.bo_num";
