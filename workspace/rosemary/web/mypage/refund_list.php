...
$tpl->define('content', "skin/member/$siteoption[skin_member]/refund_list.html");
...
$list_qry = "
			...
			where 
					A.mb_num = '$mb_num' and
					A.os_num = R.os_num
			order by R.rf_sdate desc $limit
			";
...
if ($tot_cnt > 0) {
	...
	while ($rs = mysqli_fetch_array($list_query)) {
		...
		$rs['rf_amt'] = number_format($rs['rf_amt']);
		$rs['os_text'] = $os_text;
		$rs['os_state'] = $os_state;
		$rs['rf_sdate'] = date("Y-m-d",$rs['rf_sdate']);
...
$list_page = go_page($tot_cnt, $num_per_page, $num_per_block, $page, "./refund_list.php?", $key, $searchword,$mode);
